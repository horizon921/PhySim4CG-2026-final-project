...
class TransferMode(IntEnum):
    ...
    APIC : 仿射 PIC，粒子携带仿射矩阵 C（3x3），低耗散且稳定。
    """
...
@dataclass
class FluidConfig:
    """流体（FLIP/PIC/APIC）模块参数。三维交错 MAC 网格。"""

    # --- 网格（立方体单元，dx = domain_x / res_x） ---
    res_x: int = 48
    res_y: int = 48
    res_z: int = 48
    domain_x: float = 1.0          # 世界坐标下水箱沿 x 的宽度
    # domain_y / domain_z 由 res 与 dx 推导，保证立方体单元
    ...
    pressure_iters: int = 60       # Gauss-Seidel 迭代数（use_cg=False 时）
    cg_max_iters: int = 120        # PCG 最大迭代数
    cg_tol: float = 1e-4           # PCG 相对残差阈值（视觉足够；做精度对比可调到 1e-6）
    extrapolate_iters: int = 3     # 速度外插迭代数

    # --- 粒子 ---
    particles_per_cell: int = 8    # 初始每个流体单元的粒子数（2x2x2 抖动）
    max_particles: int = 1_500_000 # 粒子缓冲上限（含发射余量）
    seed: int = 1234
    ...
    @property
    def domain_z(self) -> float:
        return self.dx * self.res_z
